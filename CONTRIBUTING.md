...
Use a concise, descriptive, imperative subject, usually within about 72 characters, without trailing punctuation. Add a short body when the reason, behavior, or architectural consequence is not clear from the subject. Before committing, inspect `git status` and the relevant diff, run appropriate checks, and exclude unrelated files.
